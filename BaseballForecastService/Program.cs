...
namespace BaseballForecastService
{
    class Program
    {
       static string outputTxt = string.Empty;

        private static void CalculateValues(double homeERA, double awayERA, int homeRuns, int awayRuns, int homeGames, int awayGames, string homeTeam, string awayTeam, List<int> homeSDEV = null, List<int> awaySDEV = null, List<int> homeHitsStdDev = null, List<int> awayHitsStdDev = null)
        {
            ...
            outputTxt += homeTeam + ": " + Math.Round(homeWinsPerc, 2) * 100 + " % " + awayTeam + ": " + Math.Round(awayWinsPerc, 2) * 100 + "% " + Environment.NewLine + "    "
        ...
        private static List<T> GetStat<T>(Helpers.SearchType type, string id = "", DateTime date = default(DateTime))
        {
            int year, month, day;
            ...
            switch (type)
            {
                case Helpers.SearchType.BaseballGame:
                    ...
                    var json = new Helpers.MyWebClient().DownloadString(url);
                ...
                case Helpers.SearchType.RunsHistory:
                    ...
                    url = @"http://mlb.mlb.com/lookup/json/named.sport_pitching_game_log_composed.bam?game_type=%27R%27&league_list_id=%27mlb%27&player_id=" + id + "&season=2019";

                    json = new Helpers.MyWebClient().DownloadString(url);
                    ...
                    if (pitchGames.GetType() == typeof(Newtonsoft.Json.Linq.JArray))
                    {
                    ...
                    {
                        p_runs.Runs.Add((int)pitchGames.SelectToken("r"));
                    }
                    x_runs.Add(p_runs);
                ...
                case Helpers.SearchType.TeamRuns:
                    ...
                    url = @"http://mlb.mlb.com/lookup/json/named.team_hitting_season_leader_master.bam?season=2019&sort_order=%27desc%27&sort_column=%27avg%27&game_type=%27R%27&sport_code=%27mlb%27&recSP=1&recPP=50";
                    json = new Helpers.MyWebClient().DownloadString(url);
        ...
        static void Main(string[] args)
        {
            ...
            var path = System.IO.Path.GetDirectoryName(Environment.GetCommandLineArgs()[0]) + @"\games.txt";
            ...
            forecast.Baseball_Game = GetStat<Helpers.BaseballGame>(Helpers.SearchType.BaseballGame, "", DateTime.Now);

            foreach (var phist in forecast.Baseball_Game)
            {
               // if (phist.AwayTeam == "Cincinnati" || phist.HomeTeam == "Cincinnati")
               // {
                 //   continue;
               // }
                var pitchingHistoryHome = new List<Helpers.RunsHistory>();
                pitchingHistoryHome = GetStat<Helpers.RunsHistory>(Helpers.SearchType.RunsHistory, phist.HomePitcherID.ToString());

                var pitchingHistoryAway = new List<Helpers.RunsHistory>();
                pitchingHistoryAway = GetStat<Helpers.RunsHistory>(Helpers.SearchType.RunsHistory, phist.AwayPitcherID.ToString());

                forecast.Runs_History.AddRange(pitchingHistoryHome);
                forecast.Runs_History.AddRange(pitchingHistoryAway);
            }

            forecast.Team_Runs = GetStat<Helpers.TeamRun>(Helpers.SearchType.TeamRuns);
            ...
            foreach (var game in forecast.Baseball_Game)
            {
              //  if (game.AwayTeam == "Cincinnati")
               ...
               // }
                ...
                {
                    var datesGames = GetStat<Helpers.BaseballGame>(Helpers.SearchType.BaseballGame, "", currentDate.AddDays(-1));
        ...
        {
            using (MailMessage mm = new MailMessage("marky.rigas@gmail.com", "marky.rigas@gmail.com"))
            {
                mm.Subject = "Baseball Forecasts";
                mm.Body = "Here ya go";
                mm.Attachments.Add(new Attachment(System.IO.Path.GetDirectoryName(Environment.GetCommandLineArgs()[0]) + @"\games.txt"));

                mm.IsBodyHtml = false;
                SmtpClient smtp = new SmtpClient();
                smtp.Host = "smtp.gmail.com";
                smtp.EnableSsl = true;
                NetworkCredential NetworkCred = new NetworkCredential("marky.rigas@gmail.com", "hardcore120984afrodi");
        ...
        private static void AppendResultToTextFile(string result)
        {
